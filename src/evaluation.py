...
def fitness (model, evaluation_granularity):
    ...
    prediction_x = build_prediction_timepoints(training_x[0], training_x[-1], evaluation_granularity)
    prediction_y = model.predict(prediction_x)[0]
    
    # TODO: factor in sigmas -> higher uncertainty = lower fitness = higher value
